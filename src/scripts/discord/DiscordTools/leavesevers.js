...
class LeaveGuilds {
    ...
    async getgroups(){
        await this.logIt("Attempting to fetch servers...")
        if (this.running == false) return
        try{
        const config = {
            method: "get",
        ...
        if (await this.checkRess(response) == false) {
            throw new Error("Failed while fetching servers!")
        }
    ...
    async leave(){
        ...
        await this.logIt(`${Blue}Attempting to leave ${name}...`)
        if (this.running == false) return
        try{
        const config = {
            method: "DELETE",
            url: `https://discord.com/api/v9/users/@me/guilds/${groups[i].id}`,
            data: {"lurking":false},
            ...
            headers: {
                ...
                scheme: 'https',
                accept: '*/*',
                path: `/api/v9/users/@me/guilds/${groups[i].id}`,
                origin: 'https://discord.com',
                referer: `https://discord.com/channels${groups[i].id}`,
                'sec-ch-ua-platform': "Windows",
                'sec-fetch-dest': 'empty',
                'sec-fetch-mode': 'cors',
                'sec-fetch-site': 'same-origin',

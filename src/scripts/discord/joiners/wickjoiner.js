...
async function wicks(token) {
    ...
    await axios(config).then(async function(response) {
        ...
        await axios(config).then(async function(response) {
            //sovles captcha
            let captcha = response.data[0].embeds[0].image.url
            console.log(`\x1b[33m[Wick Solving] \x1b[35m${captcha} \x1b[34m${await gettime()}`)
            client = new Client('14b8bbd820ffc6d1c3f46002691288ef', {
                timeout: 60000,
                polling: 2000,
                throwErrors: true
            });
            client.decode({
                ...
                console.log(`\x1b[33m[Wick] Returning Code \x1b[35m${token} \x1b[34m${await gettime()}`);
                const config = {
                    httpsAgent: agent, //proxy here
                    method: "post",
                ...
                await axios(config).then(async function(response) {
                    console.log(`\x1b[32m[Successfully Solved Wick] \x1b[35m${token} \x1b[34m${await gettime()}`)
                });
            });
        })
    }).catch(function(error) {
        console.log(`\x1b[31m[Error] Failed To Solve Wick \x1b[34m${await gettime()}`)
    });
}
